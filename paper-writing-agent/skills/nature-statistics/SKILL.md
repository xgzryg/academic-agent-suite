...
Read [the package shared rules](../../references/shared-rules.md) first. This bundled support branch audits the supplied analysis and reporting; it does not authorize computation, refitting, new correction, changed outcomes or an external statistical service. Reuse the current project method decision and actual source results. If that decision conflicts with the design, describe the specific concern without silently changing it. Use the current official requirement for the actual journal and stage; historical notes remain dated guidance. Missing data limits the affected judgement, not independent authorized editing. All `references/` paths are relative to this branch.
