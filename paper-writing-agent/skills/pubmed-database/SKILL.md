...
description: PubMed文献检索、MeSH检索式、摘要和标识批量读取、种子近似文献；内置独立stdlib执行器，支持无API key检索并保留真实检索范围与失败状态。
---
...
实际运行先读 [执行器说明](references/bundled-runner.md)，调用包根目录的 [pubmed_evidence.py](../../scripts/pubmed_evidence.py)。无需 uv、BioPython、science_skills_common、MCP 或 home .env；可用已配置环境变量中的 NCBI_API_KEY/NCBI_EMAIL，不要读取或输出凭据，不复制本机配置。所有结果输出至明确的任务目录，既有文件不覆盖。

历史资料的年份与示例标识仅作语法示例；实时事实从返回数据确认。API key 是可选项。PubMed搜索的是书目信息和索引字段，不等同于全文搜索。排序/近似分用于发现文献，不是新颖性评分。跨进程请求应由一个检索方集中执行，避免各审稿子agent并发抢同一限流配额。

本分支融合本机 pubmed-search 的简易检索以及 pubmed-database-2 的批量摘要、链接、检索终止与缺失区分知识；不带入其失效本地公共库路径、固定邮箱、摘要截断、自动装uv和home .env写入。近似查新使用 [创新性评估](../paper-novelty-assessment/SKILL.md)。其他E-utilities的参数仍见包内API参考，使用宿主HTTP或自行实现时同样遵守限速、真实来源和错误区分要求。
...
<!--
[LobsterAI 迁移元数据]
original_frontmatter: {"name": "pubmed-database", "description": "Direct REST API access to PubMed. Advanced Boolean/MeSH queries, E-utilities API, batch processing, citation management. For Python workflows, use the bundled evidence runner (Bio.Entrez). Use this for direct HTTP/REST work or custom API implementations."}
-->
