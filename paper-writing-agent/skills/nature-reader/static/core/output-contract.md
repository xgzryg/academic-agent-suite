...
- If the input is a PDF, load the bundled `paper-pdf` skill first for extraction and OCR guidance.
- If the user asks for a richer browser view, use the bundled `paper-research-artifacts` branch only as a preview layer on top of the Markdown workflow.
