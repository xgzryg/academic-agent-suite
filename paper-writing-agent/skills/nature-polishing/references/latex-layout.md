...
- [3. Orientation follows data and journal requirements](#3-orientation-follows-data-and-journal-requirements)
...
- **Check the current editable source against the approved figure:** verify the data, labels, encodings and any manual finishing. Do not regenerate an approved manually fixed figure from an older script or overwrite it. Create the requested layout revision from the actual current source, preserving scientific content; pixel identity is not a necessary test of equivalent vector exports.
- **Touch only the authorized target.** Determine whether the requested figure is main text or SI and identify its actual producing function. Preserve other approved figures; example function names are not a real project mapping.
...
## 3. Orientation follows data and journal requirements

Prefer an upright layout when it improves reading, but this is a design preference, not a verified universal Nature policy. Check the actual journal/template before ruling out landscape. Do not distort a scientifically appropriate wide figure merely to fill a portrait page.
...
- [ ] Figure aspect and source changes are authorized, preserve the scientific content and improve the actual layout (§2).
- [ ] Orientation suits the data and actual journal/template requirements (§3).
