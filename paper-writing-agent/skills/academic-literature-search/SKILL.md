...
description: 多源学术文献检索与引用管理，核验PMID/DOI、保存检索来源、去重、引用映射和用户指定格式；PubMed可用包内执行器，无需本机MCP。
---
...
1. **核实工具后调用**：若宿主确有对应 PubMed MCP，读取真实 schema 后使用；否则读 [包内 PubMed](../pubmed-database/SKILL.md) 并用独立脚本。下列 MCP 名称是来源平台示例，不能假称当前可用。
...
3. **格式由任务或目标期刊决定**；仅用户要求 GB/T 7714-2015 时采用以下示例。GB/T sequential numbering: `[1][2][3]` in-text, references numbered by order of first appearance.
4. **Journal name consistency**: Follow the requested journal/reference style; use verified full journal names or verified ISO abbreviations as that style requires. E.g., `Nature Medicine` not `Nat Med`. If MCP returns abbreviated names, expand them; if expansion is uncertain, use the name as returned.
...
| 论文参考文献（它引了谁） | **MCP** `pubmed_article_connections` (references) | 覆盖取决于工具与数据库；可核实 ELink 可用 linkname，不保证完整参考文献覆盖 |
...
def process_citations(body, sources):
    ...
    """
    # Reject unresolved citation IDs instead of silently deleting their evidence markers.
    explicit_ids = [int(n) for group in re.findall(r"\[([0-9, ]+)\]", body) for n in re.findall(r"\d+", group)]
    invalid_ids = sorted({n for n in explicit_ids if not 1 <= n <= len(sources)})
    if invalid_ids:
        raise ValueError(f"Unresolved citation IDs: {invalid_ids}; preserve markers and verify sources before renumbering")
...
2. 展示结果；PubMed仅覆盖部分预印本，不能代表bioRxiv/medRxiv全库。需要完整预印本覆盖时查询其官方服务。
...
<!--
[LobsterAI 迁移元数据]
original_frontmatter: {"name": "academic-literature-search", "description": "Academic Literature Search — 学术文献检索与引用管理"}
-->

## 适配限制

PubMed预印本覆盖不完整；期刊IF不能从Semantic Scholar引用数推导。例示引用函数只是格式起点，最终按目标规范校核；不要推测姓名拆分或把缺失作者标成真实 Anonymous。混合有DOI/无DOI记录的去重需交叉比较已知PMID和规范题名，不能只用每条记录的首选键。编号改动需保留旧新映射；无法解析的引用标记先报出，不删除来掩盖缺引。全文与摘要证据分开标记。联网失效时仍可整理用户给定文献，所有新外部事实标记待核实。
