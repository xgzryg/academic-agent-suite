...
- **Interval terminology.** A confidence interval covers a parameter; a prediction interval covers a
  ...
  The confidence level is `1 - alpha` (for example, 95% when alpha = 0.05), not alpha itself. Check which quantity the author actually means before changing the term.
...
Run one relevant pass, correct confirmed findings, and recheck the changed or unresolved items. A further pass needs a concrete new concern; do not prescribe three passes or continue until an unchanged scan is clean.
...
- Error-bar overlap alone does not determine the significance of a comparison. Identify whether bars show SD, SEM or CI, the pairing/dependence and the appropriate contrast/test. Preserve descriptive differences when supported; do not infer no difference or equivalence from overlap or a non-significant test.
...
2. One relevant terminology sweep (sections 1 and 2); recheck only changed or unresolved contexts.
...
Synchronize affected downstream artifacts within the authorized scope and list other affected files for later action. For an authorized revision package, see `../../nature-response/references/package-consistency-audit.md`.

Statistical clarification sources: [Nature Methods: Error bars](https://www.nature.com/articles/nmeth.2659) and [NIST: Confidence intervals](https://www.itl.nist.gov/div898/handbook/prc/section1/prc14.htm). These clarify interpretation, not a requirement to run new analyses.
