...
Reuse an existing terminology record when available. For a new substantial manuscript job, maintain a compact working list of recurring concepts. A short passage can use an in-context list; do not create a separate frozen contract or new file merely to edit one sentence. Canonical choices remain revisable when the author corrects them.
...
- If the user later renames a term, update occurrences within the authorized scope and the working ledger. When only a passage is authorized, identify other affected locations without silently rewriting the rest of the manuscript.
