...
> This is a retained local source snapshot. Its original verification date does not mean this package has rechecked every current requirement. Before applying an exact numeric, submission-stage or specialist policy requirement, consult the named journal’s current official page and the task’s actual applicability. If unavailable, label that item unverified and continue independent authorized writing.
...
Use the Nature-specific figure reference in `../core/figure-evidence-support.md` for final dimensions,
...
- route exact repository, accession, restriction and materials checks to
  `../core/data-availability-support.md`
