...
def visible_lines(text: str, *, latex: bool = True) -> Iterable[tuple[int, str]]:
    """Skip fenced examples; strip percent comments only for LaTeX inputs."""
    in_fence = False
    for line_number, raw_line in enumerate(text.splitlines(), 1):
        ...
        if in_fence:
            continue
        line = re.sub(r"(?<!\\)%.*$", "", raw_line) if latex else raw_line
...
def find_phrase_occurrences(
    ...
    for path, text in sources.items():
        for line_number, line in visible_lines(text, latex=path.suffix.lower() in {".tex", ".ltx"}):
