...
class ConsistencyCheckerTests(unittest.TestCase):
    def run_text(
        ...
        with tempfile.TemporaryDirectory() as directory:
            path = Path(directory) / ('manuscript' + suffix)
    ...
    def test_markdown_percent_does_not_hide_following_terms(self) -> None:
        findings = self.run_text(
            'This study reports 50% accuracy. This work describes the result.',
            CHECKER.DEFAULT_TERM_GROUPS,
            suffix='.md',
        )
        self.assertIn('TERM_VARIANTS_PRESENT', {item.code for item in findings})

    def test_latex_comment_does_not_add_visible_terms(self) -> None:
        findings = self.run_text(
            'This study reports results. % This work is only a comment.',
            CHECKER.DEFAULT_TERM_GROUPS,
        )
        self.assertEqual([], findings)

    def test_concentration_is_not_a_length(self) -> None:
        findings = self.run_text('Concentration was 35 mM; cover was 3.5 cm.')
        self.assertNotIn(
            'EQUIVALENT_LENGTH_UNIT_VARIANT', {item.code for item in findings}
        )
