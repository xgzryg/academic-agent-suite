...
When the detected section is `method`, or a whole-manuscript draft or
restructure includes Methods or Supplementary Methods, also read
[Methods directness and cross-section redundancy](../../references/methods-directness-and-redundancy.md).
Apply it while planning section content: put reproducible facts in Methods,
observed comparisons in Results, and interpretive consequences in Discussion;
do not repeat the same caveat across all three.
...
- `language` — en / zh / zh-to-en. Use the requested output language and the manifest's translation condition.
