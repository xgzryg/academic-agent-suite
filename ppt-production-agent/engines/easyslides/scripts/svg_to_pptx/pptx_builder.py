...
def _append_relationship(
    ...
    """Append a relationship irrespective of the package's XML prefix."""
    root = ET.parse(rels_path).getroot()
    rid_numbers = [int(m.group(1)) for item in root
                   if (m := re.fullmatch(r'rId(\d+)', item.get('Id', '')))]
...
def _ensure_notes_master(extract_dir: Path, content_types: str) -> str:
    ...
    if not notes_master_path.exists():
        # A complete native notes master with its own theme is required by
        # desktop PowerPoint. A schema-valid minimal master sharing the slide
        # theme can be rejected when this is the first notes master in a deck.
        seed = Presentation()
        seed.slides.add_slide(seed.slide_layouts[6]).notes_slide
        buffer = BytesIO()
        seed.save(buffer)
        with zipfile.ZipFile(buffer) as seed_zip:
            notes_master_path.write_bytes(seed_zip.read('ppt/notesMasters/notesMaster1.xml'))
            seed_rels = ET.fromstring(seed_zip.read('ppt/notesMasters/_rels/notesMaster1.xml.rels'))
            theme_rel = next(item for item in seed_rels if item.get('Type', '').endswith('/theme'))
            seed_theme = 'ppt/theme/' + Path(theme_rel.get('Target')).name
            theme_dir = extract_dir / 'ppt' / 'theme'
            theme_dir.mkdir(exist_ok=True)
            index = 1
            while (theme_dir / f'notesTheme{index}.xml').exists():
                index += 1
            new_theme_name = f'notesTheme{index}.xml'
            (theme_dir / new_theme_name).write_bytes(seed_zip.read(seed_theme))
    ...
    if not notes_master_rels_path.exists():
        notes_master_rels_path.write_text(
            create_notes_master_rels_xml(theme_name), encoding='utf-8',
        )
    ...
    for rel in pres_rels:
        if (rel.get('Type', '').endswith('/notesMaster') and
                rel.get('Target', '').lstrip('/') in
                ['notesMasters/notesMaster1.xml', 'ppt/notesMasters/notesMaster1.xml']):
    ...
    master_list = presentation_root.find(f'{{{pns}}}notesMasterIdLst')
    if master_list is None:
        master_list = ET.Element(f'{{{pns}}}notesMasterIdLst')
        slide_masters = presentation_root.find(f'{{{pns}}}sldMasterIdLst')
        position = list(presentation_root).index(slide_masters) + 1 if slide_masters is not None else 0
        presentation_root.insert(position, master_list)
    if not any(item.get(f'{{{rns}}}id') == notes_master_rid for item in master_list):
        ET.SubElement(master_list, f'{{{pns}}}notesMasterId', {f'{{{rns}}}id': notes_master_rid})
    ET.ElementTree(presentation_root).write(presentation_path, encoding='utf-8', xml_declaration=True)
    ct_root = ET.fromstring(content_types)
    ct_ns = 'http://schemas.openxmlformats.org/package/2006/content-types'
    if new_theme_name:
        ET.SubElement(ct_root, f'{{{ct_ns}}}Override',
                      {'PartName': f'/ppt/theme/{new_theme_name}',
                       'ContentType': 'application/vnd.openxmlformats-officedocument.theme+xml'})
    part = '/ppt/notesMasters/notesMaster1.xml'
    if not any(item.get('PartName') == part for item in ct_root):
        ET.SubElement(ct_root, f'{{{ct_ns}}}Override',
                      {'PartName': part, 'ContentType':
                       'application/vnd.openxmlformats-officedocument.presentationml.notesMaster+xml'})
    return ET.tostring(ct_root, encoding='unicode')
...
def _add_default_content_type(content_types: str, extension: str, content_type: str) -> str:
    ...
    if any(item.get('Extension') == ext for item in root):
        return content_types
    entry = ET.Element('{http://schemas.openxmlformats.org/package/2006/content-types}Default',
                       {'Extension': ext, 'ContentType': content_type})
    index = next((i for i, item in enumerate(root) if item.tag.endswith('}Override')), len(root))
  ...
  <Relationship Id="rId1" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/slideLayout" Target="../slideLayouts/slideLayout1.xml"/>{extra_rels}
        ...
        if enable_notes and notes_slides_created:
            ...
            for i in sorted(notes_slides_created):
                ...
                if not any(item.get('PartName') == part for item in ct_root):
                    ET.SubElement(ct_root, '{http://schemas.openxmlformats.org/package/2006/content-types}Override',
                                  {'PartName': part, 'ContentType':
                                   'application/vnd.openxmlformats-officedocument.presentationml.notesSlide+xml'})
            content_types = ET.tostring(ct_root, encoding='unicode')
            with open(content_types_path, 'w', encoding='utf-8') as f:
                f.write(content_types)
