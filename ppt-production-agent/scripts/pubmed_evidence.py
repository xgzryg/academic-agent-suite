...
class NCBIClient:
    ...
    def request(self, endpoint, params, mode='xml'):
        ...
        body = dict(params)
        body['tool'] = 'ppt-production-agent'
        ...
        for attempt in range(3):
            ...
            request = urllib.request.Request(BASE+endpoint, data=urllib.parse.urlencode(body).encode('utf-8'), headers={'User-Agent':'ppt-production-agent/1.1 (PubMed evidence)', 'Content-Type':'application/x-www-form-urlencoded'})
            try:
